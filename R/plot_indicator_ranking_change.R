...
plot_indicator_ranking_change <- function(
  ...
  if(is.null(countries_selected)){ 
    ...
    g <- dt_expanded |> 
      ...
      #geom_point(color = "#FFFFFF", size = 4) +
      geom_point(aes(size = Indicator, color = Country), alpha = 1, size = 4) +
      geom_point(color = "#FFFFFF", size = 1) +
      scale_y_reverse(
      ...
      geom_point(color = "#FFFFFF", size = 1) +
      geom_text(data = dt_expanded[Year == min(Year)],
                aes(label = Country, x = Year-0.25), hjust = 0.85, fontface = "bold", 
                color = "black", 
                size = 4) +
      geom_text(data =dt_expanded[Year == max(Year)],
                aes(label = Country, x = Year+0.25) , hjust = 0.155, fontface = "bold", 
                color = "black", 
                size = 4) +
...
plot_indicator_ranking_change_BACKGROUND <- function(
    dt, 
    ineq_indicator = c(
      "Mean", 
      "Median", 
  ...
  if(is.null(countries_selected)){ 
    ...
    dt_expanded[, CountrySelected := (ifelse(Country %chin% countries_selected, Country, ""))]
    
    g <- dt_expanded |> 
      ggplot(
        aes(x = Year, y = Rank, group = Country)
      ) + 
      geom_line( aes(x = Year, y = Rank), color = "lightgrey")+
      geom_line(
        aes( color = CountrySelected ), size = 2, alpha = 1
      )+
      #geom_point(color = "#FFFFFF", size = 4) +
      geom_point(aes(size = Indicator, color = Country), alpha = 1, size = 4) +
      geom_point(color = "#FFFFFF", size = 1) +
...
plot_indicator_ranking_change_BACKGROUND(
  dt = dt_pip, 
  ineq_indicator = "Gini", 
  countries_selected = c("South Africa", "Colombia", "United States")
)
