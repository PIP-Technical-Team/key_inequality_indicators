...
```{r pfw}
# pfw <- haven::read_dta("data/pfw_v11.dta") |>
...
#   distinct()
# 
# data <- left_join(dt_pip, 
#                   pfw, 
#                   by = c("country_code", "year", "welfare_type"),
#                   reportvar = F)
# 
# data <- data |>
#   filter(year >= 2000) |>
...
```{r create-high-ineq}
# dt_ineq <- expand.grid(country_code = data$country_code |> unique(), 
#                                year = c(2000:2022)) |> 
#   joyn::joyn(y = data |> #dt_pip |> 
#                # fsubset(year >= 2000 & year <= 2022) |> 
#                fsubset(reporting_level == "national" | country_code == "ARG") |> 
...
dt_ineq <- dt_ineq  |>
  ...
  fungroup()
  # fselect(-c(incgroup_current, "ida_current", "fcv_current"))
  # joyn::joyn(y = dt_class |> fselect(country_code = code, region_code = region_pip, incgroup_current, ida_current, fcv_current) |> 
  #              funique(), 
  #            keep = "left", 
  #            match_type = "m:1", 
  #            by = c("country_code"))
...
#   scale_colour_brewer(..., palette = custom_palette )
# }
# 
# scale_fill_brewer_d <- function(..., palette = "Dark2") {
#   scale_fill_brewer(..., palette = custom_palette)
# }
# 
# options(
...
downloadButtonRmd(
  outputId = "DownloadButtonHighIneq", 
  label    = "CSV"
)
...
output$DownloadButtonHighIneq <- downloadHandler(
  filename = function() {
