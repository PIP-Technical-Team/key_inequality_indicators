...
```{r map-data}
#load(here::here("data", "WB_geojson.Rdata"))
...
source_text <- paste0("Source: World Bank, Poverty and Inequality Platform (version ", vrs, ")")
...
```{r create-high-ineq}
dt_ineq <- haven::read_dta(file = "data/gini_intermediate_all 1.dta") |> 
...
dt_ineq <- dt_ineq  |>
  ...
  fmutate(
    giniever = gini,
    giniever = na_locf(giniever), # Forward fill
    #giniever = na.fill(giniever, fill = NA), # Ensure no trailing NAs, 
    giniever = na_focb(giniever)) |>  # Backward fill
  #fungroup() |> 
...
downloadButtonRmd <- function(outputId, 
                              label = "Download", 
                              class = NULL, ...)  {
     tags$a(id = outputId, 
...
output$DownloadButtonTrends2 <- downloadHandler(
  ...
  content = function(file) {
    write.csv(
      dt_trend_plot2() |> 
        frename(economy_code = country_code, 
...
output$DownloadButtonScatterplot <- downloadHandler(
  ...
  content = function(file) {
    write.csv(
      dt |> 
        frename(economy_name = country_name)
      , file)
  }
)
...
# Extract the colors of the Dark2 palette
dark2_palette <- brewer.pal(n = 3, name = "Dark2")

# Rearrange the colors (B, C, A)
custom_palette <- dark2_palette[c(2, 3, 1)]
...
# Select Indicator
conditionalPanel(
  condition = "input.high_ineq_plot == 'Number over time'",
  selectInput("high_ineq_region",
              label = "Plot by:",
              choices = c(
                "Global",
                c(dt_ineq$region_code |>
                  unique())[order(unique(dt_ineq$region_code))]
              ),
              selected = "Global", 
              multiple = FALSE))
...
conditionalPanel(
  condition = "input.high_ineq_plot == 'By selected group' ",
  selectInput("high_ineq_group_by_year",
              label = "Select year:",
              choices = dt_ineq$year |> unique(),
              selected = 2022, 
              multiple = FALSE))
...
conditionalPanel(
  condition = "input.high_ineq_plot == 'Pop. over time'",
  selectInput("high_ineq_pop",
              label = "Plot by:",
              choices = c(
                "Global",
                dt_ineq$region_code |>
...
```{r}
downloadButtonRmd(
  outputId = "DownloadButtonHighIneqFigure", 
  label    = "Figure data"
)
```
...
```{r}
downloadButtonRmd(
  outputId = "DownloadButtonHighIneqCountryLevel", 
...
aggregated_ineq_data <- reactive({
  
  
   if (input$high_ineq_plot == "Number over time") {
      if (!input$high_ineq_region == "Global") {
        ...
        fmutate(text_tooltip = paste0("Year: ", year, "\n",
                                      "Count: ", country_count, "\n", 
                                      "Ineq. group: ", ineq_type)) |> 
          fmutate(country_count = as.double(country_count))
  ...
  p <- dt_ineq |> 
    fsubset(year == input$high_ineq_group_by_year) |> 
    fmutate(ineq_type = factor(ineq_type, 
                               level = c("High inequality (>40)", 
                                         "Moderate inequality (30-40)",
                                         "Low inequality (<30)")))
  ...
  } else if (input$high_ineq_plot == "Pop. over time"){
    
      if (!input$high_ineq_pop == "Global") {
        p <- dt_ineq |>
          fsubset(region_code == as.character(input$high_ineq_pop))
      } else {
        p <- dt_ineq
      }
        p <- p |> 
          ...
          fgroup_by(ineq_type, year) |> 
          #fselect(ineq_type, year, country_count) |> 
          fsummarise(pop = fsum(pop)) |> 
          fungroup() |> 
          fgroup_by(year) |> 
          fmutate(popsh = round(100*pop/fsum(pop), 0)) |> 
          fungroup() |> 
          fselect(year, popsh, ineq_type) |> 
          fmutate(text_tooltip = paste0("Year: ", year, "\n",
                                        "Pop share: ", popsh, "\n", 
                                        "Ineq. group: ", ineq_type)) |> 
          fungroup()
...
renderPlotly({
  ...
  if (input$high_ineq_plot == "Number over time") {
    p <- p |> 
        ...
        geom_text(aes(label = country_count),
                ...
                color = "black") + 
        theme_classic() +  
        labs(title = paste0("Number of economies in each inequality category, ", 
                            region_matching[region_code == input$high_ineq_region, ]$region_name),
             x = "Year",
             y = "Number of economies") +
        guides(color = "none") + 
        guides(fill = guide_legend(title = "Inequality group")) + 
  ...
  p <- p |>
    ...
    geom_bar(stat     = 'identity',
             ...
             alpha    = 0.8) +
    theme_classic() +  
    labs(title = paste0("Share of economies in different inequality categories, by ", input$high_ineq_group_by),
  ...
  ggplotly(p, 
        ...
        scale_y_continuous(labels = scales::percent) +
        geom_text(aes(label = popsh),
                position  = position_fill(vjust = .5), 
                color = "black") + 
        theme_classic() +  
        labs(title = paste0("Share of population in each inequality category, ", 
                            region_matching[region_code == input$high_ineq_pop, ]$region_name),
             x = "Year",
             y = "Population share") +
        guides(color = "none") + 
        guides(fill = guide_legend(title = "Inequality group")) + 
...
reactive({
  figure_data_use <- aggregated_ineq_data()
  
  if ("use" %in% colnames(figure_data_use)) {
    figure_data_use <- figure_data_use |> 
      frename(share = use)
  }

  output$DownloadButtonHighIneqFigure <- downloadHandler(
    filename = function() {
      paste("PIP-key-ineq-indicators-highineq-figure-data-", Sys.Date(), ".csv", sep = "")
    },
    content = function(file) {
      write.csv(figure_data_use, 
                file)
    }
  )
})
...
output$DownloadButtonHighIneqCountryLevel <- downloadHandler(
  filename = function() {
    paste("PIP-key-ineq-indicators-high-ineq-", Sys.Date(), ".csv", sep = "")
  },
  content = function(file) {
    write.csv(dt_ineq |> 
                fselect(economy_code               = country_code, 
                        region_code, 
                        region_name, 
                        year, 
                        welfare_time, 
                        ...
                        reporting_level,
                        survey_gini                = gini, 
                        gini_for_inequality_groups = giniever, 
                        incgroup_FY24              = incgroup_current, 
                        ida_FY24                   = ida_current, 
                        fcv_FY24                   = fcv_current, 
                        pop,
                        ineq_group                 = ineq_type,
                        survey_comparability, 
                        comparable_spell), 
              file)
  }
...
<p style='text-align:justify;'>As a measure of shared prosperity, the World Bank tracks the Prosperity Gap and the number of economies with high inequality. This page reports the latter. An economy is considered high inequality if it has a Gini index greater than 40, based on the most recent household survey. In addition, the charts also report the number of economies with moderate (Gini index between 30 and 40, inclusive) and low inequality (Gini index less than 30). The Gini index is a measure of inequality that is bounded between 0 (a society in which everyone has the same income and hence perfect equality) and 100 (where one person has all the income and hence the most unequal society). The high inequality indicator is based on either income or consumption data, depending on the welfare measure adopted by each economy. Most high-income countries and countries in Latin America and the Caribbean use income, while the rest of the world uses consumption. For a detailed discussion, see Chapter 2 of Poverty, Prosperity, and Planet Report 2024.</p>

 

<p style='text-align:justify;'>Not all countries have a survey every year. Hence, to calculate the number of countries with high inequality, the Gini index from the most recent survey is used. The charts report the number of economies with high inequality since 2000. If a country does not have a survey in the early 2000s, their first survey after 2000 is used for those years. Economies without any surveys since 2000 have been excluded from the sample.</p>

 

<p style='text-align:justify;'>The first chart reports the number of economies in each inequality group. Users can visualize this stacked bar chart for all economies or for selected region. The second chart, shown when “Pop. over time” is selected, shows the share of the population in each inequality group. The third chart, shown when choosing “By selected group”, reports the share of economies in each inequality group for the latest year, disaggregated by the user’s chosen grouping (e.g.by the World Bank’s 2024 income group classifications, regions, etc.). The classification is held fixed for the data year 2022 using World Bank's FY 2024 income classifications.

<!-- The income group classification, using the current 2025 fiscal year World Bank classification, is held fixed at the most recent, current classifications rather than using the historical classification. To find out more about the classifications, read [this blog](https://blogs.worldbank.org/en/opendata/world-bank-country-classifications-by-income-level-for-2024-2025) or go see the updated classifications [here](https://datahelpdesk.worldbank.org/knowledgebase/articles/906519-world-bank-country-and-lending-groups?_gl=1*vq2f13*_gcl_au*MTk0MzM0NjcuMTcyMTY2NjQ2Ng..).</p> -->
...
make_table_across_countries <- function(
  ...
  setnames(
    ...
    new = c("economy", "region", "spell", "level", "type", "t10_b40", "t20_b20")                           # Clean variable names
  )
  setnames(
    dt,
  ...
  dt <- dt[
    Economy %chin% country_name_vec,         # filter by country
  ...
  setorder(dt, Economy, Region, -Year, Level, Type)
...
<!-- {data-width=500} -->
<p style='text-align:justify;'>This dashboard provides a set of interactive visualizations focused on within-country inequality. The data are from the [Poverty and Inequality Platform (PIP)](https://pip.worldbank.org/home), which is the source of the World Bank’s estimates of poverty and inequality. </p>
...
<p style='text-align:justify;'>Please cite the data as:  World Bank. (2024). Poverty and Inequality Platform (version 20240919_2017) [Data set]. World Bank Group. www.pip.worldbank.org. Accessed `r today()`. </p>
