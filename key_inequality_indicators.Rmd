...
tp1 <- ggplot(dt) +
  labs(
    title = paste("Figure 1:", input$ineq_indicatorY_3_fig1, "over time"), 
...
tp1 <- tp1 +
  geom_point(aes(x = round(year), y = transformation(IndicatorY), color = country_name), size = 3) +
...
# Add geom_point and geom_line
tp1 <- tp1 +
  geom_point(aes(x = year, y = transformation(IndicatorY), color = country_name), size = 3) +
...
p("Note: this means that if the chosen window is larger than zero, the plotted change in the selected indicator for a particular country may occur over a period longer than the chosen interval length.")
...
selectInput("background_regions",
            label    = "Select countries to compare to",
            choices  = c(
              "All", 
              dt_pip$region_name |> 
                unique()
