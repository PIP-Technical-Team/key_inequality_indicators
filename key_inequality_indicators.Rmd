...
create_data_imputed_by_countries_two_years <- function(
  ...
  dt1 <- dt1[, .(country_name, year, Indicator1, mean)]
...
```{r sidebar-table-1}
# Select Indicator
selectInput("ineq_indicator_table", 
            label = "Indicators:",
            choices = c(
              "Mean", 
              "Median", 
...
renderPlotly({
  ...
  geom_point(aes(x = Indicator2, y = country_name, color = ChangeColor, alpha = Change), shape = 2, size = 0.8)+
...
radioButtons( "select_welfare_type", 
              label = "Welfare Type", 
              choices = c("All", "Consumption", "Income"), 
              selected = "All"
)
...
selected_countries <- reactive({
  ...
  if(!input$select_reporting_level == c("All")){
    ...
    # welfare
  if(!input$select_welfare_type == c("All")){
    selected_countries <- intersect(
      selected_countries, 
...
renderPlotly({
  ...
  # Highlight countries
  sp1 <- sp1 + 
    gghighlight(country_name %chin% selected_countries())
