...
```{r create-high-ineq}
dt_ineq <- expand.grid(country_code = dt_pip[year >= 2000 & year <= 2022]$country_code |> unique(), 
                               year = c(2000:2022)) |> 
  joyn::joyn(y = dt_pip |> 
...
<p style='text-align:justify;'>The high inequality indicator on within-economy inequality uses the gini index as the measure of inequality and classifies economies with a gini greater than 40 as highly unequal. Economies with a gini below 30 is classified as low inequality, while economies with a gini index between 30 and 40 are classified as having moderate inequality. Economies without any surveys since 2000 have been excluded from the sample.</p>

<p style='text-align:justify;'>The first chart gives the number of economies per year within each inequality class. Users can visualize this stacked bar chart for all economies or for a selected region. The second chart, shown when "Pop. over time" is selected, shows the share of the population that can be found within each inequality class. The third chart, shown when choosing "By selected group", shows the share of economies within each inequality class, disaggregated by the user's chosen grouping (e.g. by the World Bank's 2024 income group classifications, regions, etc.). </p>
