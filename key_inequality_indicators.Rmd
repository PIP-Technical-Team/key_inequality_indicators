...
radioButtons("tick_all_countries_table", 
              label = "Select countries", 
              choices = c("All Countries", "Make Selection Below"), 
             selected = c("Make Selection Below")
)
...
make_table_across_countries <- function(
  ...
  selected_cols <- selected_cols |> str_replace_all(" ", "_") |> str_to_lower() |> str_to_title()
  ...
  p <- plot_ly(
    ...
    header = list(
      values = dt |> colnames() |> str_replace_all(pattern = "_", replacement = " "), # column names vector
...
output$downloadData <- downloadHandler(
  ...
  content = function(file) {
    data_to_download <- make_table_across_countries(
      dt = dt_pip,
      country_name_vec = countries_table,
...
renderPlotly({
  ...
  # Filter countries
  if(input$tick_all_countries_table == "All Countries"){
    countries_table <- pip_countries
  } else{
    
    countries_table <- c(
      input$select_countries_table, 
      pip_countries_regions[region_code %chin% input$regions_table]$country_name
    ) |> unique()
...
renderPlotly({
  ...
  geom_point(aes(x = Indicator2, y = country_name, color = ChangeColor#, alpha = Change
                 ), shape = 2, size = 0.8)+
...
radioButtons("select_reporting_level", 
...
radioButtons( "select_welfare_type", 
...
renderPlotly({
  ...
  ggplotly(sp1, dynamicTicks = TRUE, originalData = FALSE)
...
checkboxInput("log_y_3_fig1", 
              label = "Log transformation in Figure 1",  
              value = FALSE)

checkboxInput("log_y_3_fig2", 
              label = "Log transformation in Figure 2",  
              value = FALSE)
...
# Render the ggplot object using ggplotly
ggplotly(tp1)
