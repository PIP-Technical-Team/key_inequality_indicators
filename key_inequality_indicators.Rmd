...
pop <- pipr::get_aux(table = "pop", server = "qa")
pop <- pop |> 
...
# Step 1: data ----
dt_pip <- pipr::get_stats(server = "qa") |> 
  qDT()# as data.table
dt_pip[
  , 
  `:=`(
  ...
  if (input$high_ineq_plot == "Over time") {
      if (!input$high_ineq_region == "Global") {
        ...
        geom_text(aes(label = country_count),
                ...
                color = "black") + 
        theme_classic() +  
        labs(title = paste0("Number of economies in each inequality category, ", input$high_ineq_region),
             x = "Year",
             y = "Economy count",  
             linetype = "Cylinder") +
        guides(color = "none") + 
  ...
  ggplotly(p, 
      ...
      if (!input$high_ineq_pop == "Global") {
        ...
        geom_text(aes(label = popsh),
                ...
                color = "black") + 
        theme_classic() +  
        labs(title = paste0("Share of population in each inequality category, ", input$high_ineq_pop),
             x = "Year",
             y = "Population share") +
        guides(color = "none") + 
        guides(fill = guide_legend(title = "Inequality type"))
